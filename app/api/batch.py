...
def _clean(p):
    """Strip quotes that ride along with Windows 'Copy as path'."""
    return str(p).strip().strip('"').strip("'").strip() if p else p
...
@router.get("/batch/suggest_classes")
def suggest_classes(template: str):
    """Suggested class mapping for the loaded traf against a client template,
    using the same auto-match logic the run uses (exact + aliases + the
    traf's own class full names). Returns {ourClass: header}."""
    try:
        import sys as _sys
        if _ROOT not in _sys.path:
            _sys.path.insert(0, _ROOT)
        from client_template_filler import auto_match_class, _norm
        from app.core.database import query

        info = template_info(_clean(template))
        if 'error' in info:
            return info
        headers = info['class_headers']
        norm_to_orig = {_norm(h): h for h in headers}
        header_cols = {_norm(h): i for i, h in enumerate(headers)}

        rows = query("SELECT DISTINCT class_name, class_full_name FROM tracks "
                     "WHERE class_name IS NOT NULL")
        extra = {}
        our = []
        for r in rows:
            our.append(r['class_name'])
            full = r['class_full_name']
            if full and _norm(full) != _norm(r['class_name']):
                extra.setdefault(_norm(r['class_name']), []).append(_norm(full))

        suggested, unmatched = {}, []
        for c in sorted(set(our)):
            hit = auto_match_class(c, header_cols, extra)
            if hit is not None:
                suggested[c] = norm_to_orig[hit]
            else:
                unmatched.append(c)
        return {'suggested': suggested, 'unmatched': unmatched}
    except Exception as e:
        return {'error': str(e)}
...
def download(path: str):
    ...
    real = os.path.abspath(_clean(path))
